...
//I couldn't think of a smarter approach even after a few hours so, fuck this problem

mutex mtx;
map<vector<int>, int> memo;
int total_count = 0;
void process_row(int row, const vector<int>& joltage, const vector<vector<int>>& btns, int& total){
    ...
    function<int(vector<int>)> local_dfs = [&](vector<int> vals) -> int {
        ...
        if(local_memo.count(vals)) return local_memo[vals];
        ...
        function<void(int, vector<int>&, int)> partition = [&](int btn_idx, vector<int>& presses, int sum){
            ...
            for(int cnt = 0; cnt <= target; cnt++){
                ...
                if(sum + cnt > target) break; // We can get rid of unnecessary calls
            }
        };
    ...
    for(int j : joltage) cout << j << " ";
    cout << "-> " << result << " presses" << endl;
    
    if(result != INT_MAX) total += result;
}
...
int main(void){
    ...
    while(getline(file, line)){
        ...
        auto buttons_begin = sregex_iterator(line.begin(), line.end(), button_pattern);
    ...
    for(int row = 0; row < buttons.size(); row++){
        threads.emplace_back(process_row, row, ref(joltages[row]), ref(buttons[row]), ref(total));
    }
